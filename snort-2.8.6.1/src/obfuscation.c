...
static int TraverseReassembled(
        Packet *,
        int (*)(struct pcap_pkthdr *, uint8_t *, uint8_t *, uint32_t, void *),
        void *
        );
#endif
...
static ObRet TraverseObfuscationList(ObfuscationCallbackData *,
        const struct pcap_pkthdr *, const uint8_t *, ob_size_t);
static int ObfuscateStreamSegmentsCallback(struct pcap_pkthdr *,
        uint8_t *, uint8_t *, uint32_t, void *);
static ObRet GetObfuscatedPayloadCallback(const struct pcap_pkthdr *,
...
static ObRet TraverseObfuscationList(ObfuscationCallbackData *data,
        const struct pcap_pkthdr *pkth, const uint8_t *payload_data,
        ob_size_t payload_size)
{
    int i;
    ob_size_t total_offset = data->total_offset;
    ob_size_t payload_offset = 0;
    const struct pcap_pkthdr *pkth_tmp = pkth;
 ...
 *
 ******************************************************************************/
static int ObfuscateStreamSegmentsCallback(struct pcap_pkthdr *pkth,
        uint8_t *pkt, uint8_t *payload, uint32_t seq_num, void *data)
{
    ...
    ob_size_t payload_size = pkth->caplen - (payload - pkt);
 ...
 *
 ******************************************************************************/
static ObRet GetObfuscatedPayloadCallback(const struct pcap_pkthdr *pkth,
...
static void CreateObEntries(Packet *, ob_char_t, ob_size_t,
        ob_size_t, int, int);
static ObRet ObCallback(struct pcap_pkthdr *, uint8_t *, ob_char_t,
        ob_size_t, void *);
static uint8_t * GetPayloadFromFile(char *, ob_size_t *);

static int TraverseReassembled(Packet *p,
        int (*callback)(struct pcap_pkthdr *, uint8_t *, void *),
        void *user_data)
{
    ObfuscationCallbackData *callback_data =
...
static ObRet ObCallback(struct pcap_pkthdr *pkth, uint8_t *packet_data,
...
static void SegmentPayload(Packet *p)
{
    ...
    for (i = 0; i < p->dsize; i += length)
    {
        ...
        ObTestAlloc((void **)&seg->pkth, 0, sizeof(struct pcap_pkthdr));
...
int main(int argc, char *argv[])
{
    ...
    ob_size_t offset = 0;
    struct pcap_pkthdr pkth, *pkthtmp;
    Packet packet;
    ...
    pkthtmp = (struct pcap_pkthdr *)&packet.pkth;
    pkthtmp = &pkth;
