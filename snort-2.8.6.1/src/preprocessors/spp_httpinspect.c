...
static void HttpInspectCheckConfig(void)
{
...
#ifdef ZLIB
    {
        ...
        if (sfPolicyUserDataIterate(hi_config, HttpInspectExtractGzip) != 0)
        {
            ...
            {
                FatalError("http_inpsect:  Must configure a default global "
...
static int HttpInspectReloadVerify(void)
{
...
#ifdef ZLIB
    {
        ...
        if (hi_gzip_mempool != NULL)
        {
            ...
            if (defaultSwapConfig->max_gzip_sessions != defaultConfig->max_gzip_sessions)
            {
                ErrorMessage("http_inspect:  Changing max_gzip_sessions requires "
                        "a restart.\n");
                HttpInspectFreeConfigs(hi_swap_config);
                hi_swap_config = NULL;
                return -1;
