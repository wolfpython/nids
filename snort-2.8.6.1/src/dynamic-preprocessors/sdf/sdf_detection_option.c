...
int SDFOtnHandler(void *potn)
{
    ...
    while (tmp != NULL && tmp->type != RULE_OPTION_TYPE_LEAF_NODE)
    {
        ...
        {
            DynamicPreprocessorFatalMessage("Rules with SDF options cannot "
        ...
        /* Remove any pre-existing entry with this gid & sid.
           This cleans up merged OTNs due to rule revisions or multiple
           policies. */
        RemoveOptionData(head_node_to_use, sdf_data->gid, sdf_data->sid);
