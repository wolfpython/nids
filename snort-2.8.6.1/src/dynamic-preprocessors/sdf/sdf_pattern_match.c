...
/* Recurses through a SDF pattern tree, returns a pointer to the node with a
   particular sid & gid. */
static sdf_tree_node * LocateNodeByIDs(sdf_tree_node *node, uint32_t gid, uint32_t sid)
{
    uint16_t i;
    ...
    /* Recurse through children to locate sid & gid */
    for (i = 0; i < node->num_children; i++)
    {
        sdf_tree_node *ret;
        ret = LocateNodeByIDs(node->children[i], gid, sid);
        if (ret != NULL)
            return ret;
    }
    ...
    /* Base case */
    if (node->option_data &&
        node->option_data->gid == gid &&
        node->option_data->sid == sid)
    {
        return node;
    }
...
/* Removes the option data of a particular sid & gid from a SDF pattern tree.
   This gets called in the event that ParseRuleOptions() merges two OTNs,
   freeing one that we already added. */
int RemoveOptionData(sdf_tree_node *head, uint32_t gid, uint32_t sid)
{
    sdf_tree_node *target = LocateNodeByIDs(head, gid, sid);

    if (target == NULL)
        return 0;

    free(target->option_data->pii);
    free(target->option_data);
    target->option_data = NULL;
...
int AddPiiPiece(sdf_tree_node *node, char *new_pattern, SDFOptionData *data) 
{
    ...
    */
    ...
    if (*node_pattern_copy == '\0' && *new_pattern == '\0')
    {
        ...
        if (node->option_data != NULL)
        {
            DynamicPreprocessorFatalMessage("Sensitive Data rule %d:%d uses a "
                "pattern that duplicates rule %d:%d.\n", 
                data->gid, data->sid, node->option_data->gid, node->option_data->sid);
        }
        node->option_data = data;
        return 1;
    }
...
int SplitNode(sdf_tree_node *node, uint16_t split_index)
{
    ...
    new_node->option_data = node->option_data;
...
int FreePiiTree(sdf_tree_node *node)
{
    ...
    if (node->option_data)
    {
        free(node->option_data->pii);
        free(node->option_data);
    }
    free(node);
...
/* TODO: optional characters, revise syntax based on PCRE */
/* Returns a PII pointer that corresponds with the matched pattern */
static SDFOptionData * FindPiiRecursively(sdf_tree_node *node, char *buf, uint16_t *buf_index, uint16_t buflen, SDFConfig *config)
{
    uint16_t old_buf_index, pattern_index = 0;
    int node_match = 1;
    SDFOptionData *matched_pii = NULL;
    ...
    if (node_match)
    {
        ...
        while (i < node->num_children && matched_pii == NULL)
        {
            matched_pii = FindPiiRecursively(node->children[i], buf, buf_index, buflen, config);
            i++;
        }
        ...
        {
            /* No child nodes match? See if this is the matching one. */
            if (node->option_data != NULL)
            {
                /* Run eval func, return NULL if it exists but fails */
                if (node->option_data->validate_func != NULL &&
                    node->option_data->validate_func(buf, *buf_index, config) != 1)
                {
 ...
 *
 * returns: pointer to SDFOptionData for matched pattern, or NULL if no match.
 */
SDFOptionData * FindPii(sdf_tree_node *head, char *buf, uint16_t *buf_index, uint16_t buflen,
                        SDFConfig *config)
{
    uint16_t i;
    ...
    {
        SDFOptionData *data;
        data = FindPiiRecursively(head->children[i], buf, buf_index, buflen, config);
        if (data)
            return data;
    }
