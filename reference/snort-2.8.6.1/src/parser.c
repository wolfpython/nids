            ...
            if (rule_options[j].name == NULL)
            {
                ...
                if (dopt == NULL)
                {
                    ...
                    int ret = GetPreprocessorRuleOptionFuncs
                        (opts[0], &initFunc, &evalFunc, &preprocOtnHandler, &fpFunc);
...
static int TestHeader(RuleTreeNode * rule, RuleTreeNode * rtn)
{
    ...
    if (rule->sip && rtn->sip)
    {
        if (!CompareIPAddrSets(rule->sip, rtn->sip))
            return 0;
    }

    if (rule->dip && rtn->dip)
    {
        if (!CompareIPAddrSets(rule->dip, rtn->dip))
            return 0;
    }
#endif
...
void ConfigDetection(SnortConfig *sc, char *args)
{
    ...
    if (fp->configured)
    {
        ParseMessage("WARNING: Reconfiguring detection options.");
...
void ConfigMinTTL(SnortConfig *sc, char *args)
{
    ...
    min_ttl_value = SnortStrtol(args, &endptr, 0);
    if ((errno == ERANGE) || (*endptr != '\0') ||
